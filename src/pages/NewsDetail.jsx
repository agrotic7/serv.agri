...
// Ajout utilitaire pour badge "Nouveau"
...
// Utilitaire pour formater le contenu (version simple)
function formatContent(content) {
  if (!content) return '';
  // Remplace les sauts de ligne par <br>
  let html = content.replace(/\n/g, '<br />');
  // Optionnel : gestion simple des listes, titres, blockquote
  html = html.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>'); // gras markdown
  html = html.replace(/__([^_]+)__/g, '<u>$1</u>'); // souligné markdown
  html = html.replace(/\* (.+)/g, '<li>$1</li>'); // listes à puce
  html = html.replace(/<li>(.+)<\/li>/g, '<ul><li>$1</li></ul>'); // entoure d'une ul si besoin
  html = html.replace(/# (.+)/g, '<h2>$1</h2>'); // titre niveau 2
  html = html.replace(/> (.+)/g, '<blockquote>$1</blockquote>'); // citation
  return html;
}
...
function NewsDetail() {
  ...
  return (
    ...
    >
      ...
      {/* HEADER FLOTTANT */}
      <div className="news-detail-header-pro news-detail-header-float">
        <div className="news-detail-header-left">
          <img src={AUTHOR.avatar} alt={AUTHOR.name} className="news-detail-author-avatar-pro" />
          <div>
            <span className="news-detail-author-pro">{AUTHOR.name}</span>
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              <span className="news-detail-date-pro-badge">{news.date}</span>
              {news.category && <span className="news-detail-category-badge">{news.category}</span>}
              {isNew(news.date) && <span className="news-badge-new">Nouveau</span>}
            </div>
          </div>
        </div>
        <div className="news-detail-header-right">
          <div className="news-detail-views-pro">
      ...
      <motion.div 
        className="news-detail-image-container-pro news-detail-image-animated"
        initial={{ scale: 1.04 }}
        animate={{ scale: 1 }}
        transition={{ duration: 1.1, type: 'spring' }}
      >
        <img src={news.image_url} alt={news.title} className="news-detail-image-pro" />
        <div className="news-detail-image-overlay">
          <h1 className="news-detail-title-pro">{news.title}</h1>
          {news.image_caption && <span className="news-detail-image-caption">{news.image_caption}</span>}
        </div>
      </motion.div>
      <div className="news-detail-content-pro">
        <div className="news-detail-excerpt-pro">{news.excerpt}</div>
        <div className="news-detail-full-content-pro" dangerouslySetInnerHTML={{__html: news.content}}></div>
        {news.category && (
          <div className="news-detail-category-pro">
            <span className="category-label-pro">Catégorie :</span>
            <span className="category-value-pro">{news.category}</span>
          </div>
        )}
        {news.tags && Array.isArray(news.tags) && news.tags.length > 0 && (
          <div className="news-detail-tags-pro">
            <span className="tags-label-pro">Tags :</span>
            {news.tags.map((tag, idx) => (
              <span className="news-tag-pro" key={idx}>{tag}</span>
            ))}
          </div>
        )}
        <div className="news-detail-share-pro">
          <span>Partager :</span>
          <button className="share-btn-pro facebook" onClick={() => handleShare('facebook')} title="Partager sur Facebook" aria-label="Partager sur Facebook">
          ...
          <button className="share-btn-pro twitter" onClick={() => handleShare('twitter')} title="Partager sur Twitter" aria-label="Partager sur Twitter">
          ...
          <button className="share-btn-pro linkedin" onClick={() => handleShare('linkedin')} title="Partager sur LinkedIn" aria-label="Partager sur LinkedIn">
          ...
          <button className="share-btn-pro link" onClick={handleCopyLink} title="Copier le lien" aria-label="Copier le lien">
            <svg width="22" height="22" fill="none" stroke="#888" strokeWidth="2" viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 1 7 7l-3 3a5 5 0 0 1-7-7"/><path d="M14 11a5 5 0 0 0-7-7l-3 3a5 5 0 0 0 7 7"/></svg>
          </button>
          {copied && <span className="news-link-copied">Lien copié !</span>}
        </div>
      </div>

        {/* Section commentaires interactive avancée */}
        <section className="news-comments-section-pro">
          <h2 className="comments-title-pro">{comments.length} Commentaire{comments.length>1?'s':''}</h2>
          ...
          {commentLoading && comments.length === 0 ? (
            ...
            <div className="comments-list-pro">
              ...
              {comments
                ...
                .map((c) => (
                  <div className="comment-item-pro" key={c.id}>
                    <div className="comment-avatar-pro">
                      <img src={c.avatar_url || defaultAvatar(c.author)} alt={c.author} />
                    </div>
                    <div className="comment-content-pro">
                      <span className="comment-author-pro">{c.author}</span>
                      <span className="comment-date-pro">{formatDate(c.created_at)}</span>
                      <div className="comment-text-pro">{c.content}</div>
                      ...
                      {comments.filter(r => r.parent_id === c.id).length > 0 && (
                        <div className="comment-replies-pro">
                          {comments.filter(r => r.parent_id === c.id).map(r => (
                            <div className="comment-item-pro reply" key={r.id}>
                              <div className="comment-avatar-pro">
                                <img src={r.avatar_url || defaultAvatar(r.author)} alt={r.author} />
                              </div>
                              <div className="comment-content-pro">
                                <span className="comment-author-pro">{r.author}</span>
                                <span className="comment-date-pro">{formatDate(r.created_at)}</span>
                                <div className="comment-text-pro">{r.content}</div>
                                <div className="comment-likes-row">
                                  <button
        ...
        {/* Articles similaires avancés */}
        {similarNews.length > 0 && (
          <section className="news-similar-section-pro">
            <h2 className="news-similar-title-pro">Articles similaires</h2>
            <div className="news-similar-list-pro">
              {similarNews.map(item => (
                <div className={`news-similar-card-pro${isNew(item.date)?' news-similar-card-new':''}`} key={item.id} onClick={() => navigate(`/actualites/${item.id}`)}>
                  <div className="news-similar-img-wrap-pro">
                    <img src={item.image_url} alt={item.title} className="news-similar-img-pro" />
                    {isNew(item.date) && <span className="news-similar-badge-new">Nouveau</span>}
                  </div>
                  <div className="news-similar-content-pro">
                    <span className="news-similar-date-pro">{item.date}</span>
                    <h3 className="news-similar-title-card-pro">{item.title}</h3>
                  </div>
                </div>
              ))}
            </div>
