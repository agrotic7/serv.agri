...
function AdminNews() {
  ...
  const confirmDelete = async () => {
    ...
    if (toDeleteId) {
      setLoading(true);
      await supabase.from('news').delete().eq('id', toDeleteId);
      fetchNews();
      setLoading(false);
      setToDeleteId(null);
    }
      ...
      {showDeleteModal && (
        <div className="admin-modal-overlay" onClick={cancelDelete}>
          <div className="admin-modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: '3rem', marginBottom: 8, color: '#dc2626' }}>
            ...
            <h3 style={{ fontWeight: 700, color: '#1a3c1f' }}>Supprimer cette actualité ?</h3>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16 }}>
              ...
              <button className="cancel-edit-btn" onClick={cancelDelete}>Annuler</button>
            </div>
          </div>
        </div>
      )}
