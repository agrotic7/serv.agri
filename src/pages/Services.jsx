...
// Composant pour charger la vidéo de manière différée (Lazy Load)
function ServiceMedia({ s }) {
  const ref = useRef(null);
  // La vidéo ne se charge que si elle est sur le point d'apparaître à l'écran (marge de 200px)
  const isInView = useInView(ref, { once: true, margin: "200px" });

  return (
    <div className="service-img-refonte" style={{ position: 'relative' }} ref={ref}>
      {s.video ? (
        isInView ? (
          <video
          ...
          <img src={s.img} alt={s.title} className="service-video-refonte" style={{ objectFit: 'cover' }} />
        )
      ) : (
...
export default function Services() {
  ...
  return (
    <div>
      <Helmet>
        <title>Nos Services | SERVAGRI — Agriculture de Précision au Sénégal</title>
        <meta name="description" content="Découvrez les solutions SERVAGRI : irrigation intelligente, Big Data agronomique, optimisation de l'eau, formation et accompagnement pour une agriculture de précision au Sénégal." />
      </Helmet>
      {/* HERO avec vidéo de fond, overlay et animation strictement identique à l'accueil */}
      <div className="realisation-section-container">
        <section className="realisation-hero">
          ...
          <div className="realisation-hero-content hero-pro-content">
            ...
            >
              Des services <span className="hero-pro-title-green">innovants</span><br />
            ...
            >
              Optimisez votre exploitation grâce à la technologie, l'accompagnement humain et l'expertise terrain.
            </motion.p>
            <motion.a
              href="#services-list"
            ...
            >
              ...
              <span className="hero-pro-arrow">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="24" height="24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
              </span>
            </motion.a>
          </div>
        </section>
        ...
        <div className="services-grid-refonte">
          {services.map((s, i) => (
            <motion.div className="service-card-refonte" key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} transition={{ duration: 0.5, delay: i * 0.12 }} whileHover={{ scale: 1.05, boxShadow: "0 16px 40px rgba(46,125,50,0.18)" }}>
              {/* Le composant ServiceMedia gère le chargement paresseux de la vidéo */}
              <ServiceMedia s={s} />

              {/* Affichage de l'icône dynamique */}
              <div className="service-icon-refonte">{typeof s.icon === 'string' ? (ICONS[s.icon] || <FiCheckCircle />) : (s.icon || <FiCheckCircle />)}</div>
              {/* Badge dynamique */}
              {s.badge && <div className={`service-badge ${s.badgeClass || s.badgeclass}`}>{s.badge}</div>}
              {/* Nouveau */}
